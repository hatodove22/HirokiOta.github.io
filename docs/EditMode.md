...
## Slug 仕様（ニュース）
- 形式: kebab-case の ASCII 英小文字 + ハイフンのみ（正規表現: ^[a-z0-9-]{1,80}$）
- 生成: タイトルから自動生成（記号/空白はハイフンへ、連続ハイフンは1つに圧縮、先頭/末尾ハイフンは削除）
- 重複: 既存衝突がある場合は -2, -3, ... と採番
- 日付: 当面は自動付与しない。必要になった場合は YYYY-MM-DD- を prefix する運用に変更可能（このドキュメントを更新）
