...
- [ ] Edit Mode: プレビューのMarkdownレンダリングをMarkdown出力基盤
  - 2025-10-10 Plan B: markdown-it -> generateJSON -> generateHTML -> DOMPurify パイプラインでNewsプレビューを再配線し、H1/H2/リスト反映とJSONラウンドトリップ整合を確認。
  - 2025-10-10 プレビュー表示: Tailwind Typography 非導入のため、独自 `.prose` スタイルを追加し見出し/リスト/引用の体裁を復元。（tiptap-markdown + markdown-it）で再実装し、エディタとの挙動差異を解消。
  - 2025-10-10 Plan B: markdown-it -> generateJSON -> generateHTML -> DOMPurify パイプラインでNewsプレビューを再配線し、H1/H2/リスト反映とJSONラウンドトリップ整合を確認。
